...
class SyncedVisualizer:
    ...
    def _compute_calculus(self):
        ...
        self.rhythmic_drive = np.zeros_like(_log_abs_deriv)
        _env = 0.0
    ...
    def _build_rms_derivative_panel(self, ax):
        ...
        # Percussiveness: log-domain peak-decay of |d(RMS)/dt|, instant attack, 2s decay
        # Empirical: ambient < 1.37, beat > 1.63, remap [0.97, 2.29] → [0, 1]
        pd = np.clip((self.rhythmic_drive - 0.97) / 1.32, 0.0, 1.0)
        ax2.plot(self.times, pd, color='#FFB74D', linewidth=1.5, alpha=0.8, label='rhythmic drive')

        ax2.set_ylim([0, 1.05])
        ax2.set_ylabel('rhythmic drive', fontsize=9)
        ax2.tick_params(axis='y', labelsize=8)
        ax2.legend(loc='upper right', framealpha=0.7, fontsize=7)

        ax.set_xlim([0, self.duration])
        ax.set_ylabel('dRMS/dt')
        ax.set_title('RMS Derivative + Percussiveness', fontsize=11)
        ax.grid(True, alpha=0.2)
        if not hasattr(self, 'axes'):
            self.axes = []
        self.axes.append(ax)
