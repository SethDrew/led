...
async function loadPanel() {
    ...
    if (currentTab === 'recorder') {
        ...
        document.getElementById('stemStatus').style.display = 'none';
        document.getElementById('controlsHint').innerHTML =
            'Record and play back synchronized audio + LED visuals';
        if (typeof recorderStartPolling === 'function') recorderStartPolling();
        return;
    }
    recorderPanel.style.display = 'none';
...
// ── Recorder tab (mic_bridge.py HTTP API) ──────────────────────

const RECORDER_API = '/api/recorder';
const recorderState = {
    polling: null,
    audioCtx: null,
    audioBuffer: null,
...
async function recorderPollStatus() {
    ...
    // mic level: rms is 0-65535, map to percentage (log scale)
...
function recorderStopPolling() {
    if (recorderState.polling) {
        clearInterval(recorderState.polling);
        recorderState.polling = null;
    }
}
...
document.getElementById('recorderRecBtn').addEventListener('click', async () => {
    ...
    if (!data) {
        recorderEl('recorderState').textContent = 'OFFLINE';
        recorderEl('recorderState').className = 'recorder-state';
        alert('Recorder unavailable. Plug in the bridge ESP32 via USB and reload.');
        return;
    }
...
document.getElementById('recorderPlayBtn').addEventListener('click', async () => {
    ...
    // fetch WAV and play locally, also tell backend to replay LED frames
...
// start/stop polling when tab switches
const _origSwitchTab = switchTab;
switchTab = function(tabId) {
    _origSwitchTab(tabId);
    if (tabId === 'recorder') recorderStartPolling();
    else recorderStopPolling();
};
