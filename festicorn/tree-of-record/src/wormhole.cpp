/*
 * Wormhole — entrance piece to the tree of record. Slow ambient sparkle
 * fade, rust + soft whites.
 *
 * Classic ESP32 (biolum-b), 6 × WS2812B RGB strips on GPIO 4, 15, 17,
 * 5, 18, 19. No sensors, no radio — a quiet standalone ambient piece.
 * Build/flash with env:wormhole.
 *
 * Each spark is an independent envelope: ease in over seconds, hold,
 * ease out even slower. Colors draw from a rust family (warm
 * orange-brown, slightly jittered per spark) with occasional soft warm
 * whites. Delta-sigma dithering smooths the slow tails below 8-bit.
 */
...
// occupied pixels are skipped, so steady state solves
// N = RATE * (1 - N/900) * ~7.9s mean lifetime. 80/s settles near ~41%
// of 900 LEDs lit (~370, ~12/m at 30 LED/m).
#define SPARK_RATE        80.0f
...
void setup() {
    ...
    // scatter ignitions and age them randomly into their envelopes.
    int preSeed = (int)(NUM_STRIPS * LEDS_PER_STRIP * 0.41f);
    ...
    Serial.printf("[wormhole] %u strips x %u LEDs, strip1 pin %d\n",
                  NUM_STRIPS, (unsigned)LEDS_PER_STRIP, STRIP1_PIN);
}
