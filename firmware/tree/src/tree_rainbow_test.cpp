/*
 * TREE + RAINBOW COMPARISON TEST
 *
 * Tree: Sap flow animation on 3 strips (pins 27/13/14, 197 LEDs)
 * Test strip: OKLCH constant-L vs variable-L rainbow on pin 32
 *
...
static const uint8_t oklchConstL[256][3] PROGMEM = {
...
// Red (~30 deg) L~0.52, purple (~300 deg) L~0.38, green/cyan/yellow L=0.75.
...
// Rainbow cycle time
static const uint32_t CYCLE_TIME_MS = 8000;

void setup() {
    Serial.begin(115200);
    Serial.println("=== TREE + RAINBOW TEST ===");
    ...
    testStrip.clear();
    testStrip.show();
    Serial.print("Test strip: ");
    Serial.print(TEST_STRIP_LEDS);
    Serial.print(" LEDs on pin ");
    Serial.println(TEST_STRIP_PIN);
...
void loop() {
    ...
    // --- Test strip: OKLCH variable-L rainbow (winner) ---
    uint32_t ms = millis();
    uint16_t n = testStrip.numPixels();
    uint16_t baseHue = (uint16_t)((uint64_t)(ms % CYCLE_TIME_MS) * 65536ULL / CYCLE_TIME_MS);
    ...
    delay(16); // ~60fps (tree show() happens inside sap.render())
}
